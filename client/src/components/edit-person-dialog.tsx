...
// Use the Person type from shared schema but with narrowed types
...
export function EditPersonDialog({ open, onOpenChange, person, onSuccess }: EditPersonDialogProps) {
  ...
  const updatePersonMutation = useMutation({
    mutationFn: async (data: PersonFormValues) => {
      const res = await apiRequest("PUT", `/api/people/${person.id}`, data);
      if (!res.ok) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update contact information for {person.firstName} {person.lastName}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      {person.email !== field.value && person.email && "Changing email will require verification"}
                      {person.isEmailVerified && person.email === field.value && "✓ Verified"}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        {person.phoneNumber !== field.value && person.phoneNumber && "Changing phone number will require verification"}
                        {person.isPhoneVerified && person.phoneNumber === field.value && "✓ Verified"}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
