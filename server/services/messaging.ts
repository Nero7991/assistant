...
Now, please respond to this user message: "${context.userResponse}"`;
  ...
  processLLMResponse(content: string): {
    ...
    try {
      ...
      // Try to parse the JSON and handle emoji characters
      try {
        ...
        console.log("Initial JSON parse failed:", jsonError);
        
        // Try different methods to sanitize content
        try {
          // Method 1: Replace emoji and control characters with empty string using Unicode ranges
          // Unicode surrogate pairs used for emoji
          let sanitizedContent = cleanContent.replace(/[\uD800-\uDFFF]/g, "");
          
          // Method 2: Additional cleanup for other potential problematic characters
          // Standard approach for removing emoji without requiring unicode flags
          sanitizedContent = sanitizedContent
            .replace(/[\u1F300-\u1F6FF]/g, '') // Remove emoticons 
            .replace(/[\u1F900-\u1F9FF]/g, '') // Remove supplemental symbols
          ...
          // Method 3: Last resort - Try a more aggressive approach with a JSON repair function
          try {
            // Simple JSON repair function that removes problematic sections
            let repairAttempt = cleanContent
            ...
            console.log("Successfully parsed JSON after aggressive repair");
            ...
            // As a last resort, fall back to the original message
